          ...
          >
            <span className="flex items-center gap-2">
              ...
              {productOrders.length > 0 && (
                <span className="bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 text-xs px-1.5 py-0.5 rounded-full font-mono font-bold">
                        ...
                        >
                          <div className="space-y-4 flex-1">
                            <div className="flex flex-wrap items-center gap-3">
                              ...
                              <span className={`font-mono text-xs font-semibold px-2 py-0.5 rounded capitalize ${
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-sans text-slate-300">
                              <div className="space-y-2">
                                ...
                                <div className="flex items-start gap-2">
                                  ...
                                  <span>{booking.address} <span className="text-xs text-slate-400 font-mono">({booking.zipCode})</span></span>
                                </div>
                              </div>

                              <div className="space-y-2 border-t sm:border-t-0 sm:border-l border-slate-800 pt-2 sm:pt-0 sm:pl-4">
                                <div>
                                  <span className="text-slate-400 font-mono text-xs uppercase block">House size:</span>
                                  <span className="font-semibold">{booking.bedrooms} Bed, {booking.bathrooms} Bath • <span className="text-xs text-slate-400">{booking.sqFtRange}</span></span>
                                </div>
                                {booking.addons.length > 0 && (
                                  <div>
                                    <span className="text-slate-400 font-mono text-xs uppercase block">Add-ons:</span>
                                    <span className="text-xs text-slate-400">{booking.addons.join(", ")}</span>
                                  </div>
                                )}
                                <div>
                                  <span className="text-slate-400 font-mono text-xs uppercase block">Frequency:</span>
                                  <span className="font-semibold text-sky-400 capitalize">{booking.frequency}</span>
                                </div>
                                {booking.paymentMethod && (
                                  <div>
                                    <span className="text-slate-400 font-mono text-xs uppercase block">Payment Method:</span>
                            ...
                            {booking.entryInstructions && (
                              <div className="bg-slate-950 px-3.5 py-2.5 rounded-lg border border-slate-850 text-xs text-slate-400 leading-normal">
                                <span className="font-mono text-slate-400 uppercase text-xs block mb-0.5">Entry Instructions:</span>
                                "{booking.entryInstructions}"
                              </div>
                            )}
                            ...
                            {booking.status === "completed" && (booking.beforePhoto || booking.afterPhoto) && (
                              <div className="space-y-2 pt-2">
                                <span className="text-slate-400 font-mono text-xs uppercase block">Proof of Clean:</span>
                                  ...
                                  {booking.beforePhoto && (
                                    <div className="space-y-1">
                                      <div className="h-20 w-28 bg-slate-950 rounded-lg overflow-hidden border border-slate-855 relative">
                                        ...
                                        <span className="absolute bottom-1 right-1 bg-black/70 text-xs font-mono font-bold text-red-400 px-1 py-0.2 rounded">BEFORE</span>
                                      </div>
                                    </div>
                                  )}
                                  {booking.afterPhoto && (
                                    <div className="space-y-1">
                                      <div className="h-20 w-28 bg-slate-950 rounded-lg overflow-hidden border border-emerald-500/30 relative">
                                        ...
                                        <span className="absolute bottom-1 right-1 bg-emerald-950/80 text-xs font-mono font-bold text-emerald-400 px-1 py-0.2 rounded">AFTER</span>
                                      </div>
                                    </div>
                                  )}
                                </div>
                          ...
                          <div className="flex flex-row md:flex-col justify-between items-end md:items-end md:justify-between border-t md:border-t-0 md:border-l border-slate-800 pt-4 md:pt-0 md:pl-6 shrink-0 md:w-44 text-right">
                            <div className="space-y-1 text-right">
                              <span className="text-slate-400 font-mono text-xs uppercase block">Total Cost:</span>
                              <span className="text-white font-mono text-base sm:text-lg font-bold block">${booking.price.toFixed(2)}</span>
                              <span className="text-emerald-400 font-mono text-xs block font-semibold">SSP {(booking.price * 1300).toLocaleString()}</span>
                              {booking.tipAmount > 0 && (
                                <div className="text-right">
                                  <span className="block font-mono text-xs text-sky-400 font-semibold">+ ${booking.tipAmount.toFixed(2)} Tip</span>
                                  <span className="block font-mono text-xs text-sky-500/80 font-medium">+ SSP {(booking.tipAmount * 1300).toLocaleString()}</span>
                                </div>
                              )}
                            </div>
                                ...
                                <>
                                  ...
                                  {allowed ? (
                                    ...
                                    <div className="space-y-1 text-right">
                                      ...
                                      <span className="text-xs text-red-400 font-mono block">Less than 24h: call support</span>
                                    </div>
                                  )}
                                </>
                              )}
                    ...
                    >
                      <div className="space-y-4 flex-1">
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <span className="font-mono text-xs font-bold px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700 uppercase">
                            {order.paymentMethod}
                          </span>
                          <span className={`font-mono text-xs font-semibold px-2 py-0.5 rounded capitalize ${
                        ...
                        <div className="text-xs text-slate-300 space-y-2">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-slate-800">
                            <div>
                              <span className="text-slate-400 font-mono text-xs uppercase block">Paid via Phone:</span>
                              <span className="font-semibold font-mono text-slate-300">{order.phonePaid}</span>
                            </div>
                            <div>
                              <span className="text-slate-400 font-mono text-xs uppercase block">Delivery Address:</span>
                      ...
                      <div className="flex flex-row md:flex-col justify-between items-end md:items-end md:justify-between border-t md:border-t-0 md:border-l border-slate-800 pt-4 md:pt-0 md:pl-6 shrink-0 md:w-44 text-right">
                        <div className="space-y-1 text-right">
                          <span className="text-slate-400 font-mono text-xs uppercase block">Total Paid:</span>
                          <span className="text-white font-mono text-lg font-bold">
                            {order.currencyPaid === "USD" 
                              ? `$${order.totalUSD.toFixed(2)}` 
                              : `SSP ${order.totalSSP.toLocaleString()}`}
                          </span>
                          <span className="block text-xs text-slate-400 font-mono">Ref: {order.transactionRef}</span>
                        </div>
              ...
              {!isEditingProfile ? (
                ...
                >
                  ...
                  <div className="flex justify-between items-end z-10 font-mono">
                    <div>
                      <span className={`block text-xs uppercase font-mono ${
                ...
                <form onSubmit={handleUpdateProfile} className="space-y-3 font-sans text-xs">
                  <div className="space-y-1">
                    <label className="text-xs text-slate-400 font-mono uppercase">Profile Holder Name</label>
                    <input 
                      type="text" 
                      required
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-slate-400 font-mono uppercase">Payment Channel</label>
                    <select
                  ...
                  {profileEditForm.method !== "Cash" && (
                    <div className="space-y-1">
                      <label className="text-xs text-slate-400 font-mono uppercase">Mobile Number</label>
                      <input 
                        type="text" 
                        required
                  ...
                  {profileUpdateSuccess && (
                    <p className="text-emerald-400 font-mono text-xs text-center animate-pulse pt-1">Profile updated successfully!</p>
                  )}
        ...
        {selectedBookingForReschedule && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-2">
                <span className="text-sky-400 font-mono text-xs uppercase font-bold tracking-wider block">RESCHEDULE APPOINTMENT</span>
              ...
              <div className="space-y-4 text-xs font-sans">
                <div className="space-y-1">
                  <label className="text-slate-400 font-mono text-xs uppercase">New Date</label>
                  <input 
                    type="date" 
                    value={newDate}
                ...
                <div className="space-y-1">
                  <label className="text-slate-400 font-mono text-xs uppercase block mb-1">Preferred Time Window</label>
                  <div className="grid grid-cols-1 gap-2">
                    {timeSlots.map((slot) => (
                      <button
                        ...
                        className={`w-full py-2.5 px-3 rounded-lg border text-left font-mono text-xs transition-all ${
                          newTimeSlot === slot
                ...
                {rescheduleError && (
                  <p className="text-red-400 font-mono text-xs text-center">{rescheduleError}</p>
                )}
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="text-center space-y-3">
                ...
                <div className="space-y-1">
                  <span className="text-emerald-400 font-mono text-xs uppercase font-bold tracking-wider block">SUPPORT OUR CREW</span>
              ...
              {!tipSuccess ? (
                <div className="space-y-6">
                  ...
                  {/* Custom tip input */}
                  <div className="space-y-2">
                    <label className="text-xs text-slate-400 font-mono uppercase block text-center">Custom Tip Amount</label>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-1 text-left">
                <span className="text-emerald-400 font-mono text-xs uppercase font-bold tracking-wider block">LIVE VEHICLE DISPATCH TRACKER</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs bg-slate-950 border border-slate-850 p-4 rounded-2xl font-sans text-left">
                <div className="space-y-1 border-b sm:border-b-0 sm:border-r border-slate-850 pb-2 sm:pb-0 sm:pr-4">
                  <span className="text-xs text-slate-400 font-mono uppercase">ESTIMATED ARRIVAL</span>
                  <div className="text-lg font-bold text-emerald-400 font-mono animate-pulse">{etaMinutes} mins</div>
                  <span className="text-xs text-slate-400 block">Juba traffic included</span>
                </div>
                <div className="space-y-1 border-b sm:border-b-0 sm:border-r border-slate-850 pb-2 sm:pb-0 sm:pr-4">
                  <span className="text-xs text-slate-400 font-mono uppercase">DISPATCH CREW</span>
                  <div className="font-semibold text-white">Deng Lual &amp; Team Alpha</div>
                  <span className="text-xs text-slate-400 block">3 Certified HSE Technicians</span>
                </div>
                <div className="space-y-1">
                  <span className="text-xs text-slate-400 font-mono uppercase">VEHICLE DETAILS</span>
                  <div className="font-semibold text-white">Toyota Land Cruiser</div>
                  <span className="text-xs text-slate-400 font-mono block">Juba Plate: SSD-309A</span>
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="border-b border-slate-800 pb-4 space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-mono mt-0.5">CLEAN WORLD INC. JUBA OFFICE</p>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-xs font-sans">
                <div>
                  <span className="text-xs text-slate-400 font-mono uppercase block">TRANSACTION REF</span>
                  <span className="font-mono font-bold text-white">{selectedOrderForInvoice.transactionRef}</span>
                </div>
                <div>
                  <span className="text-xs text-slate-400 font-mono uppercase block">DATE</span>
                ...
                <div>
                  <span className="text-xs text-slate-400 font-mono uppercase block">PAYMENT METHOD</span>
                  <span className="font-semibold text-white">
                ...
                <div>
                  <span className="text-xs text-slate-400 font-mono uppercase block">PHONE PAID</span>
              ...
              <div className="border-t border-b border-slate-800 py-4 space-y-2.5">
                <span className="text-xs text-slate-400 font-mono uppercase block">ORDER ITEMS</span>
                <div className="space-y-2">
                  {selectedOrderForInvoice.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center text-xs">
                      <div className="flex flex-col">
                        <span className="font-medium text-white">{item.product.name}</span>
                        <span className="text-xs text-slate-400 font-mono">{item.product.unit} (x{item.quantity})</span>
                      </div>
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex justify-between items-center text-sm font-semibold pt-2 border-t border-slate-850">
                  ...
                  <div className="text-right">
                    ...
                    <span className="block text-xs text-slate-400 font-mono">Simulated Exchange Rate: 1 USD = 1,300 SSP</span>
                  </div>
                </div>
              </div>

              <div className="bg-slate-950/40 border border-slate-800 p-3.5 rounded-xl text-xs text-slate-400 leading-normal space-y-1.5">
                <div>
                  <span className="font-mono text-slate-400 text-xs uppercase block">DELIVERY TARGET:</span>
                  <span className="font-semibold text-white">{selectedOrderForInvoice.deliveryLocation} District</span>
                </div>
                <div>
                  <span className="font-mono text-slate-400 text-xs uppercase block">STREET ADDRESS:</span>
