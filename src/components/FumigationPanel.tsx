  ...
  return (
    <div className="relative flex items-center justify-center w-full">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <span className="absolute top-1 left-1/2 -translate-x-1/2 text-xs font-mono font-bold text-violet-300/80 tracking-widest uppercase">VECTOR</span>
        <span className="absolute top-1/2 right-0 -translate-y-1/2 text-xs font-mono font-bold text-violet-400/70 tracking-widest uppercase">COLONY</span>
        <span className="absolute bottom-1 left-1/2 -translate-x-1/2 text-xs font-mono font-bold text-indigo-400/60 tracking-widest uppercase">NETWORK</span>
        <span className="absolute top-1/2 left-0 -translate-y-1/2 text-xs font-mono font-bold text-violet-300/50 tracking-widest uppercase">DENGUE</span>
      </div>
    </div>
  );
}
...
export default function FumigationPanel({
  ...
  const [selectedVectors, setSelectedVectors] = useState<string[]>(["Mosquito & Malaria Defense"]);
  ...
  const VECTOR_SERVICES = [
    { icon: <Droplets className="w-4 h-4" />, label: "Mosquito & Malaria Defense" },
    { icon: <Bug className="w-4 h-4" />,     label: "Termite & Insect Control" },
    { icon: <AlertTriangle className="w-4 h-4" />, label: "Rodent & Pest Proofing" },
    { icon: <Shield className="w-4 h-4" />,  label: "Perimeter Property Protection" },
  ];
  ...
  const handleResetForm = () => {
    ...
    setSelectedVectors(["Mosquito & Malaria Defense"]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 animate-fadeIn" id="fumigation-panel">
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-semibold tracking-[0.2em] uppercase" style={{ color: "#c9adf3" }}>
        ...
        <div className="grid grid-cols-3 gap-3">
          {CREDIBILITY_STATS.map(({ value, label, sub }) => (
            <div key={label} className="pl-3 py-1 flex flex-col border-l-2" style={{ borderColor: "rgba(201,173,243,0.4)" }}>
              ...
              <span className="text-xs font-mono text-slate-400 uppercase tracking-wide leading-none mt-1.5">{sub}</span>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-col gap-3 shrink-0">
            {THREAT_NODES.map(node => (
              <div key={node.id} className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-slate-400">{node.abbr}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <span className="text-xs font-mono uppercase tracking-[0.2em] text-slate-400 flex items-center gap-2">
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="rounded-xl border p-5 space-y-4" style={{ background: "rgba(15,23,42,0.6)", borderColor: "rgba(201,173,243,0.14)" }}>
          <span className="text-xs font-mono uppercase tracking-[0.2em] text-slate-400 block">Treatment Intensity Class</span>
          <div className="space-y-2">
            {TREATMENT_TIERS.map(tier => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-slate-400 font-sans leading-snug">{tier.desc}</p>
                </button>
              );
            })}
          </div>
        ...
        <div className="rounded-xl border px-5 py-4 space-y-3 shadow-md" style={{ background: "rgba(15,23,42,0.5)", borderColor: "rgba(201,173,243,0.12)" }}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono uppercase tracking-[0.2em] text-slate-400">Deployed Equipment</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {["ULV Thermal Foggers", "Pneumatic Sprayers", "Eco-safe Larvicides"].map((eq, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1.5 text-xs font-mono font-medium px-2.5 py-1 rounded-md border transition-colors"
          ...
          >
            ...
            {!submitSuccess ? (
              <form onSubmit={handleFormSubmit} className="space-y-6">
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-mono text-xs uppercase tracking-widest font-extrabold" style={{ color: "#c9adf3" }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-xs text-slate-400 font-mono uppercase tracking-wide block">Facility / Compound Name *</label>
                      <input
                    ...
                    <div className="space-y-1">
                      <label className="text-xs text-slate-400 font-mono uppercase tracking-wide block">Facility Area (sq ft) *</label>
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-slate-500 font-mono text-xs">SIZE</span>
                          <span className="font-mono font-bold text-white text-xs px-2 py-0.5 rounded border" style={{ background: "rgba(15,23,42,0.9)", borderColor: "rgba(201,173,243,0.15)" }}>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs text-slate-400 font-mono uppercase tracking-wide block">Treatment Intensity</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs text-slate-400 font-mono uppercase tracking-wide block">Juba Operating District *</label>
                      ...
                      <div className="mt-2 p-3 rounded-xl font-mono text-xs text-slate-400 space-y-1.5" style={{ background: "rgba(15,23,42,0.7)", border: "1px solid rgba(201,173,243,0.08)" }}>
                        ...
                        <div className="text-xs text-slate-500 leading-normal border-t pt-1" style={{ borderColor: "rgba(201,173,243,0.08)" }}>{activeLocObj.description}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <label className="text-xs text-slate-400 font-mono uppercase tracking-wide block">Required Treatment Vectors</label>
                      <div className="space-y-2">
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs truncate max-w-[200px] sm:max-w-xs" style={{ color: isChecked ? "white" : "#94a3b8" }}>{srv.label}</span>
                              </div>
                              <span
                    ...
                    <div className="space-y-2.5 text-xs font-sans">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <label className="text-xs text-slate-400 font-mono uppercase block">Focal Point Name *</label>
                          <input
                        ...
                        <div className="space-y-1">
                          <label className="text-xs text-slate-400 font-mono uppercase block">Official Email *</label>
                          <input
                      ...
                      <div className="space-y-1">
                        <label className="text-xs text-slate-400 font-mono uppercase block">Direct Contact Phone *</label>
                        <input
                      ...
                      <div className="space-y-1">
                        <label className="text-xs text-slate-400 font-mono uppercase block">Threat Assessment Notes</label>
                        <textarea
                ...
                <div className="pt-4 border-t flex flex-col sm:flex-row items-center justify-between gap-4" style={{ borderColor: "rgba(201,173,243,0.1)" }}>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-left font-mono">
                      <span className="text-xs text-slate-500 uppercase block leading-none">TREATMENT CLASS</span>
                      <span className="text-xs font-bold tracking-wide" style={{ color: "#c9adf3" }}>{activeTier.label} — {areaSize.toLocaleString()} sq ft</span>
                    </div>
                  </div>
              ...
              <div className="py-8 text-center space-y-6 max-w-md mx-auto">
                ...
                <div className="space-y-1">
                  <span className="font-mono text-xs uppercase tracking-widest font-extrabold block" style={{ color: "#c9adf3" }}>FACILITY SECURED — DISPATCH REGISTERED</span>
                ...
                <div className="p-5 rounded-2xl text-left font-mono text-xs text-slate-300 space-y-3.5" style={{ background: "rgba(15,23,42,0.9)", border: "1px solid rgba(201,173,243,0.15)" }}>
