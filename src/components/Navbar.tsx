  ...
  return (
    <div className={`z-50 w-full transition-all duration-300 ${isGlassCapsule ? "sticky top-2 px-4 md:px-8 py-2" : "sticky top-0"}`}>
      ...
      <nav 
        className={`transition-all duration-300 ${
          isDoubleDecker 
            ...
            : isGlassCapsule
              ? "max-w-7xl mx-auto bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl shadow-2xl shadow-emerald-500/5 hover:border-emerald-500/20"
      ...
      >
              ...
              {navItems.map((item) => (
                ...
                >
                  ...
                  {activeView === item.view && !isCyberGrid && (
                    <span className="absolute bottom-1 left-3 right-3 h-[2px] bg-emerald-400 rounded-full" />
                  )}
                </button>
              ))}
            </div>
        ...
        {/* Mobile Drawer */}
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-slate-900 py-4 px-4 space-y-4 animate-fadeIn bg-slate-950 rounded-b-2xl" id="nav-mobile-drawer">
